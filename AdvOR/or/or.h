...
#define IDENTITY_FLAG_EXPIRE_TRACKED_HOSTS 1
#define IDENTITY_FLAG_EXPIRE_CIRCUITS 2
#define IDENTITY_FLAG_DESTROY_CIRCUITS 4
#define IDENTITY_FLAG_LIST_SELECTION 8
#define IDENTITY_FLAG_GENERATE_SEEDS 16
#define IDENTITY_FLAG_EXPIRE_HTTP_COOKIES 32
#define IDENTITY_FLAG_DELETE_FLASH_COOKIES 64
#define IDENTITY_FLAG_DELETE_SILVERLIGHT_COOKIES 128
#define IDENTITY_FLAG_RANDOMIZE_WMPLAYER_ID 256
#define IDENTITY_FLAG_DELETE_HTTP_COOKIES 512
#define IDENTITY_FLAG_REINIT_KEYS 1024
#define IDENTITY_FLAG_AVOID_GEOIP_BANS 2048
#define IDENTITY_FLAG_AUTO_CHANGE_IP 4096
#define IDENTITY_FLAG_NO_MESSAGEBOX 32768
#define IDENTITY_FLAGS_ALL 0xffff

#define DIR_FLAG_RANDOM_AUTHORITY 1
#define DIR_FLAG_NO_AUTO_UPDATE 2
#define DIR_FLAG_WAIT_MORE_DESCRIPTORS 4
#define DIR_FLAG_FAKE_LOCAL_TIME 16
#define DIR_FLAG_USE_ROUTER_TIME 32
#define DIR_FLAG_HTTP_PROXY 64
#define DIR_FLAG_HTTP_AUTH 128
#define DIR_FLAG_HTTPS_PROXY 256
#define DIR_FLAG_HTTPS_AUTH 512
#define DIR_FLAG_NTLM_PROXY 1024
#define DIR_FLAG_SOCKS_AUTH 2048
#define DIR_FLAGS_ALL 0xffff
...
#define PROXY_NTLM_WANT_CONNECT_OK 7
...
#define DIR_CONN_STATE_PROXY_HANDSHAKING 7
...
#define CIRCUIT_PRIORITY_HIGHEST -3
#define CIRCUIT_PRIORITY_HIGHER -2
#define CIRCUIT_PRIORITY_HIGH -1
#define CIRCUIT_PRIORITY_NORMAL 0
#define CIRCUIT_PRIORITY_LOW 1
#define CIRCUIT_PRIORITY_LOWER 2
#define CIRCUIT_PRIORITY_LOWEST 3
...
typedef struct connection_t {
  ...
  unsigned int plugin_read:1;
  unsigned int plugin_write:1;
  unsigned int hs_plugin:1;
  ...
  int processed_from_inbuf;
  char *unprocessed,*incoming;
  int64_t unprocessed_len,incoming_len;
  int64_t expecting_data,need_data;
  int expecting_no_data,expecting_trailers,need_trailers;
  int last_boundary,need_boundary;
  char *last_host;
  buf_t *inbuf; /**< Buffer holding data read over this connection. */
  buf_t *outbuf; /**< Buffer holding data to write over this connection. */
  size_t outbuf_flushlen; /**< How much data should we try to flush from the
                           * outbuf? */
  time_t timestamp_lastcircuit;
  ...
  DWORD lParam[MAX_PLUGIN_CONNECTION_PARAMS];
...
#define MIN_CONSTRAINED_TCP_BUFFER 0
#define MAX_CONSTRAINED_TCP_BUFFER 1024*1024  /* 256k */
...
#define CONFIRM_EXIT_ONLY_RELEASE 2
#define CONFIRM_VERIFY_CONNECTIONS 4
#define CONFIRM_CLOSE_CONNECTIONS 8
...
typedef struct {
  ...
  enum {
    ...
    CMD_VERIFY_CONFIG, CMD_RUN_UNITTESTS, CMD_START, CMD_MINIMIZE
  } command;
  ...
  config_line_t *NotifyFilter;
  config_line_t *DebugFilter;
  config_line_t *BannedHosts;
  config_line_t *QuickStart;
  config_line_t *SynchronizeExit;
  config_line_t *Plugins;
  config_line_t *PluginOptions;
  ...
  long TrackHostExitsExpire; /**< Number of seconds until we expire an
                             * addressmap */
  ...
  char *DirProxy; /**< hostname[:port] to use as http proxy, if any. */
  tor_addr_t DirProxyAddr; /**< Parsed IPv4 addr for http proxy, if any. */
  uint16_t DirProxyPort; /**< Parsed port for http proxy, if any. */
  char *DirProxyAuthenticator; /**< username:password string, if any. */
  int DirProxyProtocol;

  char *ORProxy; /**< hostname[:port] to use as https proxy, if any. */
  tor_addr_t ORProxyAddr; /**< Parsed IPv4 addr for https proxy, if any. */
  uint16_t ORProxyPort; /**< Parsed port for https proxy, if any. */
  char *ORProxyAuthenticator; /**< username:password string, if any. */
  int ORProxyProtocol;

  char *CorporateProxy; /**< hostname[:port] to use as https proxy, if any. */
  tor_addr_t CorporateProxyAddr; /**< Parsed IPv4 addr for https proxy, if any. */
  uint16_t CorporateProxyPort; /**< Parsed port for https proxy, if any. */
  char *CorporateProxyDomain; /**< username:password string, if any. */
  char *CorporateProxyAuthenticator; /**< username:password string, if any. */
  int CorporateProxyProtocol;
...
#define SAFELOG_SCRUB_NONE 0
#define SAFELOG_SCRUB_ALL 1
#define SAFELOG_SCRUB_RELAY 2

#define ALLOW_DOT_EXIT 1
#define ALLOW_DOT_ONION 2
...
#define LOG_PROTOCOL_WARN (get_options()->ProtocolWarnings ? \
  ...
  char	*LocalHost;
  char	*Language;
  int HotkeyRestore,HotkeyNewIdentity,HotkeyIntercept,HotkeyRelease,HotkeyHideAll,HotkeyRestoreAll;
  ...
  int ExitMaxUptime,ExitMaxSeen,ExitSeenFlags;
  ...
  int DirFlags;
  int IdentityFlags;
  long IdentityAutoChange;
  int HTTPFlags;
  int HTTPAgent;
  int HTTPOS;
  int RegionalSettings;
  int ResolveTimeout;
  int MaxDlFailures;
  int MaxFileAge;
  int CircuitPathLength;
  int NumIntroPoints,IntroCircRetryPeriod,MaxCircsPerPeriod,MaxRendFailures,MaxRendTimeout;
  long MaxTimeDelta,BestTimeDelta,FavoriteExitNodesPriority;

  config_line_t *BannedHeaders;
...
#define SOCKS_COMMAND_SELECT_ROUTER 0xE0
...
struct socks_request_t {
  ...
  char reply[MAX_SOCKS_REPLY_LEN]; /**< Write an entry into this string if
                                    ...
                                    */
  char *address; /**< What address did the client ask to
                                       connect to/resolve? */
...
#define EXCLUSIVITY_DIRCONN 4
#define EXCLUSIVITY_INTERNAL 5
...
#define EXIT_SEEN_FLAG_ENABLED 1
#define EXIT_SEEN_FLAG_SAVE_STATS 2
#define EXIT_SEEN_FLAG_EXCLUDE_GEOIP_BANS 4

typedef DWORD (WINAPI *LPFN1)(DWORD);
typedef BOOL (WINAPI *LPFN2)(DWORD,HANDLE,char *);
typedef BOOL (WINAPI *LPFN3)(DWORD,HANDLE,DWORD,DWORD,char *,DWORD);
typedef DWORD (WINAPI *LPFN4)(void);
typedef	BOOL (WINAPI *LPFN5)(DWORD,DWORD,char *,uint16_t *);
typedef	BOOL (WINAPI *LPFN6)(HWND,HWND);
typedef	BOOL (WINAPI *LPFN7)(HWND,DWORD,DWORD,DWORD,DWORD,char *);
typedef	uint32_t (WINAPI *LPFN8)(uint32_t,DWORD);
typedef int (WINAPI *LPFN9)(char *,int,DWORD);
typedef void (WINAPI *LPFN10)(char *,DWORD);
typedef	BOOL (WINAPI *LPFN11)(HWND,HWND,void *);
typedef HANDLE (WINAPI *LPFN12)(DWORD,HANDLE,DWORD,DWORD,char *,DWORD);
typedef HANDLE (WINAPI *LPFN13)(HANDLE,LPTHREAD_START_ROUTINE,LPARAM);
typedef int (WINAPI *LPFN14)(DWORD,DWORD,char *,uint32_t,uint32_t);

extern char fullpath[MAX_PATH+1];
extern char last_dir_status[256];
extern const char *versions[];
extern long delta_t;
extern long best_delta_t;
extern char pipeName[];
extern LPFN5 GetConnInfo;
extern char *_version;

#define MAX_PID_LIST 50
#define SELECT_SET_EXIT 0
#define SELECT_ENTRY 1
