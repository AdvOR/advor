...
nameserver_probe_failed(struct nameserver *const ns) {
	...
	if (add_timeout_event(ns, (struct timeval *) timeout) < 0) {
		log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_TIMER_ERROR),debug_ntop((struct sockaddr *)&ns->address));
		/* ???? Do more? */
	}
}
...
nameserver_failed(struct nameserver *const ns, const char *msg) {
	...
	log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NS_FAILED),debug_ntop((struct sockaddr *)&ns->address),msg);
	global_good_nameservers--;
	tor_assert(global_good_nameservers >= 0);
	if (global_good_nameservers == 0) {
		log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NS_FAILED_2));
	}
...
nameserver_up(struct nameserver *const ns) {
	if (ns->state) return;
	log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NS_IS_BACK),debug_ntop((struct sockaddr *)&ns->address));
	del_timeout_event(ns);
	ns->state = 1;
	ns->failed_times = 0;
	ns->timedout = 0;
...
request_finished(struct evdns_request *const req, struct evdns_request **head) {
	...
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REMOVING_TIMEOUT),(unsigned long) req);
...
reply_handle(struct evdns_request *const req, u16 flags, u32 ttl, struct reply *reply) {
	...
	if (flags & 0x020f || !reply || !reply->have_answer) {
		...
		case DNS_ERR_SERVERFAILED:
			...
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SERVERFAILED),debug_ntop((struct sockaddr *)&req->ns->address));
			break;
		default:
...
static INLINE int name_parse(u8 *packet, int length, int *idx, char *name_out, size_t name_out_len)
{	int name_end = -1;
	int j = *idx;
	int ptr_count = 0;
...
reply_parse(u8 *packet, int length) {
	...
	if(j + 12 > length)	return -1;
	memcpy(&_t, packet + j, 2); j += 2; trans_id = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; flags = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; questions = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; answers = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; authority = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; additional = ntohs(_t);
	...
	{	reply.type = req->request_type;
		...
		{	/* the question looks like <label:name><u16:type><u16:class> */
			tmp_name[0] = '\0';
			cmp_name[0] = '\0';
			k = j;
			...
			else if (strcasecmp(tmp_name, cmp_name) == 0)
				name_matches = 1;
			j += 4;
			if(j >= length)
			{	name_matches = 0;
		...
		{	/* now we have the answer section which looks like <label:name><u16:type><u16:class><u32:ttl><u16:len><data...> */
			...
			{	u16 type, class;
				tmp_name[0] = '\0';
				if((name_parse(packet, length, &j, tmp_name, sizeof(tmp_name))<0) || (j + 2+2+4+2 > length))
				{	name_matches = 0;
					break;
				}
				else
				{	memcpy(&_t, packet + j, 2); j += 2; type = ntohs(_t);
					memcpy(&_t, packet + j, 2); j += 2; class = ntohs(_t);
					memcpy(&_t32, packet + j, 4); j += 4; ttl = ntohl(_t32);
					memcpy(&_t, packet + j, 2); j += 2; datalength = ntohs(_t);
					...
					{	if (req->request_type != TYPE_PTR)
						...
						strlcpy(reply.data.ptr.name, tmp_name,sizeof(reply.data.ptr.name));
						ttl_r = MIN(ttl_r, ttl);
						reply.have_answer = 1;
						break;
					}
	...
	if (req)	reply_handle(req, flags, 0, NULL);
	return -1;
}
...
/* callback. */
static int request_parse(u8 *packet, ssize_t length, struct evdns_server_port *port, struct sockaddr *addr, socklen_t addrlen)
{
	int j = 0;	/* index into packet */
	u16 _t;	 /* used by the macros */
	...
	if (j + 2+2+2+2+2+2 > length)	return -1;
	memcpy(&_t, packet + j, 2); j += 2; trans_id = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; flags = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; questions = ntohs(_t);
	memcpy(&_t, packet + j, 2); j += 2; answers = ntohs(_t);
	...
	{	for(i = 0; i < questions; ++i)
		{	u16 type, class;
			...
			size_t namelen;
			if((name_parse(packet, (int)length, &j, tmp_name, sizeof(tmp_name))<0) || (j + 2+2 > length))
				break;
			memcpy(&_t, packet + j, 2);j += 2;type = ntohs(_t);
			memcpy(&_t, packet + j, 2);j += 2;class = ntohs(_t);
...
nameserver_read(struct nameserver *ns) {
	...
	for (;;) {
		...
		if (!sockaddr_eq(sa, (struct sockaddr*)&ns->address, 0)) {
			log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_ADDRESS_MISMATCH),debug_ntop(sa));
			return;
		}
		ns->timedout = 0;
...
server_port_read(struct evdns_server_port *s) {
	...
	for (;;) {
		...
		if (r < 0) {
			...
			if (error_is_eagain(err)) return;
			log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_REQUEST_READ_ERROR),tor_socket_strerror(err),err);
			return;
		}
...
server_port_flush(struct evdns_server_port *port)
{
		...
		if (r < 0) {
			...
			if (error_is_eagain(err))
				return;
			log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_RESPONSE_WRITE_ERROR),tor_socket_strerror(err),err);
		}
		if (server_request_free(req)) {
			/* we released the last reference to req->port. */
			return;
                } else {
                        tor_assert(port->pending_replies != req);
	...
	if (event_add(&port->event, NULL) < 0) {
		log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_EVENT_ADD_ERROR));
		/* ???? Do more? */
	}
}
...
nameserver_write_waiting(struct nameserver *ns, char waiting) {
	...
	if (event_add(&ns->event, NULL) < 0) {
		log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_EVENT_ADD_ERROR_2),debug_ntop((struct sockaddr *)&ns->address));
		/* ???? Do more? */
	}
}
...
/* */
static off_t dnsname_to_labels(u8 *const buf, size_t buf_len, off_t j,const char *name, const size_t name_len,struct dnslabel_table *table)
{	const char *end = name + name_len;
	...
	/* the labels must be terminated by a 0. It's possible that the name ended in a . in which case the zero is already there */
...
static int evdns_request_data_build(const char *const name, const size_t name_len,const u16 trans_id, const u16 type, const u16 class,u8 *const buf, size_t buf_len)
{	off_t j = 0;	/* current offset into buf */
	u16 _t;	 /* used by the macros */

	if(j + 2+2+2+2+2+2 > (off_t)buf_len)	return (-1);
	_t = htons(trans_id);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(0x0100);memcpy(buf + j, &_t, 2);j += 2;	/* standard query, recusion needed */
	_t = htons(1);memcpy(buf + j, &_t, 2);j += 2;		/* one question */
	_t = htons(0);memcpy(buf + j, &_t, 2);j += 2;		/* no answers */
	_t = htons(0);memcpy(buf + j, &_t, 2);j += 2;		/* no authority */
	_t = htons(0);memcpy(buf + j, &_t, 2);j += 2;		/* no additional */
	j = dnsname_to_labels(buf, buf_len, j, name, name_len, NULL);
	if(j < 0)	return (int)j;
	else if(j + 2+2 > (off_t)buf_len)	return (-1);
	_t = htons(type);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(class);memcpy(buf + j, &_t, 2);j += 2;
	return (int)j;
}
...
evdns_add_server_port(tor_socket_t socket, int is_tcp, evdns_request_callback_fn_type cb, void *user_data)
{
	...
	tor_assert(!is_tcp); /* TCP sockets not yet implemented */
	port->socket = socket;
	port->refcnt = 1;
	port->choked = 0;
	port->closing = 0;
...
{	unsigned char buf[1500];
	...
	/* Set response bit and error code; copy OPCODE and RD fields from question; copy RA and AA if set by caller. */
	flags = req->base.flags;
	flags |= (0x8000 | err);
	dnslabel_table_init(&table);
	_t = htons(req->trans_id);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(flags);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(req->base.nquestions);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(req->n_answer);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(req->n_authority);memcpy(buf + j, &_t, 2);j += 2;
	_t = htons(req->n_additional);memcpy(buf + j, &_t, 2);j += 2;
	...
	{	const char *s = req->base.questions[i]->name;
		...
		if(j + 2+2 > (off_t)buf_len)
		{	truncated = 1;
			break;
		}
		_t = htons(req->base.questions[i]->type);
		memcpy(buf + j, &_t, 2);
		j += 2;
		_t = htons(req->base.questions[i]->dns_question_class);
		memcpy(buf + j, &_t, 2);
		j += 2;
	}
	...
	{	struct server_reply_item *item;
		...
		{	r = dnsname_to_labels(buf, buf_len, j, item->name, strlen(item->name), &table);
			...
			{	j = r;
				...
				{	if(j+item->datalen+2 > (off_t)buf_len)
						truncated = 1;
					else
					{	_t = htons(item->datalen);
						memcpy(buf + j, &_t, 2);
						j += 2;
	...
	if(truncated)	buf[2] |= 0x02; /* set the truncated bit. */
...
{
	tor_assert(port);
	tor_assert(!port->refcnt);
	tor_assert(!port->pending_replies);
	if (port->socket > 0) {
...
evdns_request_timeout_callback(int fd, short events, void *arg) {
	...
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REQUEST_TIMEOUT),(unsigned long) arg);
...
evdns_request_transmit(struct evdns_request *req) {
	...
	default:
		...
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REQUEST_SET_TIMEOUT),(unsigned long) req);
		if (add_timeout_event(req, &global_timeout) < 0) {
			log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_TIMER_ERROR_2),(unsigned long) req);
			/* ???? Do more? */
		}
		req->tx_count++;
		req->transmit_me = 0;
...
nameserver_send_probe(struct nameserver *const ns) {
	...
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SENDING_PROBE),debug_ntop((struct sockaddr *)&ns->address));
										...
										socklen_t addrlen)
{
	...
	if (addr) {
		tor_assert(addrlen <= (socklen_t)sizeof(global_bind_address));
...
static int sockaddr_is_loopback(const struct sockaddr *addr)
...
static int _evdns_nameserver_add_impl(const struct sockaddr *address,socklen_t addrlen)
{	/* first check to see if we already have this nameserver */
	...
	{	do
		{	if (sockaddr_eq(address, (struct sockaddr *)&server->address, 1))
			{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_DUPLICATE));
				return 3;
			}
			server = server->next;
	...
	{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_ADDRLEN_TOO_LONG),(int)addrlen);
		return 2;
	}
	...
	if(ns->socket < 0)	err = 1;
	else
	{	u_long nonblocking = 1;
		ioctlsocket(ns->socket, FIONBIO, &nonblocking);
		if(global_bind_addr_is_set && !sockaddr_is_loopback((struct sockaddr*)&global_bind_address) && (bind(ns->socket, (struct sockaddr *)&global_bind_address,global_bind_addrlen) < 0))
		{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_BIND_ERROR));
			err = 2;
		}
		else if (connect(ns->socket, address, addrlen) != 0)
		{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_CONNECT_ERROR));
			err = 2;
		}
		else
		{	memcpy(&ns->address, address, addrlen);
			...
			if (event_add(&ns->event, NULL) < 0)
			{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_EVENT_ERROR));
				err = 2;
			}
			else
			{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_ADDED),debug_ntop(address));
				...
				{	ns->next = server_head->next;
					ns->prev = server_head;
					server_head->next = ns;
					if(server_head->prev == server_head)	server_head->prev = ns;
				}
				global_good_nameservers++;
				return 0;
			}
	...
	CLEAR(ns);
	mm_free(ns);
	log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NS_NOT_ADDED),debug_ntop(address),err);
	return err;
}
...
evdns_nameserver_ip_add(const char *ip_as_string) {
	...
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_ADDING),ip_as_string);

	cp = strchr(ip_as_string, ':');
	if (*ip_as_string == '[') {
		size_t len;
		if (!(cp = strchr(ip_as_string, ']'))) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_ERROR));
			return 4;
		}
		len = cp-(ip_as_string + 1);
		if (len > sizeof(buf)-1) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_BUFFER_TOO_SMALL));
			return 4;
		}
		...
		if (cp - ip_as_string > (int)sizeof(buf)-1) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_BUFFER_TOO_SMALL_2));
			return 4;
		}
	...
	if (port_part == NULL) {
		...
		if (port <= 0 || port > 65535) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_PORT_OUT_OF_RANGE),port_part);
			return 4;
		}
	}

	/* Tor-only.  needs a more general fix. */
	tor_assert(addr_part);
	if (is_ipv6) {
		...
		if (1 != tor_inet_pton(AF_INET6, addr_part, &sin6.sin6_addr)) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_INET_PTON_FAILED),addr_part);
			return 4;
		}
...
static struct evdns_request *request_new(int type, const char *name, int flags,evdns_callback_type callback, void *user_ptr)
{	const char issuing_now = (global_requests_inflight < global_max_requests_inflight) ? 1 : 0;
	...
	struct evdns_request *const req = (struct evdns_request *) mm_malloc(sizeof(struct evdns_request) + request_max_len);
	char namebuf[256];
	int rlen;
	(void) flags;
	if(!req) return NULL;
	...
	{	unsigned i;
		...
		{	if(ISALPHA(namebuf[i]))
			{	if ((randbits[i >> 3] & (1<<(i%7))))	namebuf[i] = TOLOWER(namebuf[i]);
				else					namebuf[i] = TOUPPER(namebuf[i]);
			}
		}
		name = namebuf;
	}
...
int evdns_resolve_ipv4(const char *name, int flags,
	...
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_RESOLVE_REQUEST),name);
...
int evdns_resolve_reverse(const struct in_addr *in, int flags, evdns_callback_type callback, void *ptr) {
	...
	snprintf(buf, sizeof(buf), "%d.%d.%d.%d.in-addr.arpa",
			...
			(int)(u8)((a>>16)&0xff),
			(int)(u8)((a>>24)&0xff));
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REVDNS_REQUEST),buf);
...
int evdns_resolve_reverse_ipv6(const struct in6_addr *in, int flags, evdns_callback_type callback, void *ptr) {
	...
	tor_assert(cp + strlen("ip6.arpa") < buf+sizeof(buf));
...
search_request_new(int type, const char *const name, int flags, evdns_callback_type user_callback, void *user_arg) {
	tor_assert(type == TYPE_A || type == TYPE_AAAA);
	...
	if (req->search_state) {
		...
		if (req->search_index >= req->search_state->num_domains) {
			...
			if (string_num_dots(req->search_origname) < req->search_state->ndots) {
				...
				log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_RAW_QUERY),req->search_origname);
				if (newreq) {
		...
		new_name = search_make_new(req->search_state, req->search_index, req->search_origname);
				if (!new_name) return 1;
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NOW_TRYING),new_name,req->search_index);
...
evdns_set_option(const char *option, const char *val, int flags)
{
	if (!strncmp(option, "ndots:", 6)) {
		...
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_NDOTS),ndots);
		if (!global_search_state) global_search_state = search_state_new();
		...
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_TIMEOUT),timeout);
		global_timeout.tv_sec = timeout;
	} else if (!strncmp(option, "max-timeouts:", 12)) {
		const int maxtimeout = strtoint_clipped(val, 1, 255);
		if (maxtimeout == -1) return -1;
		if (!(flags & DNS_OPTION_MISC)) return 0;
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_TIMEOUT_2),maxtimeout);
		global_max_nameserver_timeout = maxtimeout;
	} else if (!strncmp(option, "max-inflight:", 13)) {
		const int maxinflight = strtoint_clipped(val, 1, 65000);
		if (maxinflight == -1) return -1;
		if (!(flags & DNS_OPTION_MISC)) return 0;
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_INFLIGHT),maxinflight);
		...
		if (!(flags & DNS_OPTION_MISC)) return 0;
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_RETRIES),retries);
		global_max_retransmits = retries;
	} else if (!strncmp(option, "randomize-case:", 15)) {
		int randcase = strtoint(val);
		if (!(flags & DNS_OPTION_MISC)) return 0;
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_SETTING_RANDOMIZE_CASE),randcase);
...
{	struct stat st;
	...
	char *start;
	int err = 0;
	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_RESOLV_CONF_PARSE),filename);
	...
	if(fstat(fd, &st))	err = 2;
	else if(!st.st_size)
	{	evdns_resolv_set_defaults(flags);
		err = (flags & DNS_OPTION_NAMESERVERS) ? 6 : 0;
	}
	else if(st.st_size > 65535)	err = 3;	/* no resolv.conf should be any bigger */
	else if(!(resolv = (u8 *) mm_malloc((size_t)st.st_size + 1)))
		err = 4;
	else
	{	n = 0;
		...
		{	resolv[n] = 0;	 /* we malloced an extra byte; this should be fine. */
			...
			if(!server_head && (flags & DNS_OPTION_NAMESERVERS))	/* no nameservers were configured. */
...
static int load_nameservers_with_getnetworkparams(void)
{	/* Based on MSDN examples and inspection of	 c-ares code. */
	...
	{	log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NO_IPHLPAPI_DLL));
		status = -1;	/* right now status = 0, doesn't that mean "good" - mikec */
	}
	else if(!(fn = (GetNetworkParams_fn_t) GetProcAddress(handle, "GetNetworkParams")))
	{	log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_IPHLPAPI_WITHOUT_API));
		status = -1;	/* same as above */
	}
	else if(!(buf = mm_malloc(size)))	status = 4;
	else
	{	fixed = buf;
		r = fn(fixed, &size);
		...
		{	if(r != ERROR_SUCCESS)
			{	mm_free(buf);
				...
				{	fixed = buf;
					r = fn(fixed, &size);
					if(r != ERROR_SUCCESS)
					{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_IPHLPAPI_CALL_FAILED));
						status = -1;
					}
				}
			}
			...
			{	added_any = 0;
				...
				{	r = evdns_nameserver_ip_add_line(ns->IpAddress.String);
					if(r)
					{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_NOT_ADDED_2),(ns->IpAddress.String),(int)GetLastError(),r);
						status = r;
					}
					else
					{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NS_ADDED),ns->IpAddress.String);
						added_any++;
					}
					ns = ns->Next;
				}
				if(!added_any)
				{	log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NO_NS_ADDED));
...
#define TRY(k, name)													\
	if (!found && config_nameserver_from_reg_key(k,name) == 0) {		\
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_FOUND_NS),#k,name);		\
		found = 1;														\
	} else if (!found) {												\
		log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_NO_NS_FOUND),			\
			#k,#name);													\
	}
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WIN_NS_NT_KEY, 0,
			...
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REGISTRY_ERROR),(int)GetLastError());
			return -1;
		}
		r = RegOpenKeyEx(nt_key, "Interfaces", 0,
						 ...
						 &interfaces_key);
		if (r != ERROR_SUCCESS) {
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REGISTRY_ERROR_2),(int)GetLastError());
			return -1;
		}
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WIN_NS_9X_KEY, 0,
			...
			log(EVDNS_LOG_DEBUG,get_lang_str(LANG_LOG_EVENTDNS_REGISTRY_ERROR_3),(int)GetLastError());
			return -1;
		}
	...
	if (found == 0) {
		log(EVDNS_LOG_WARN,get_lang_str(LANG_LOG_EVENTDNS_NO_NS_FOUND_2));
	}
...
main(int c, char **v) {
	...
	if (c<2) {
		log(EVDNS_LOG_WARN,"syntax: %s [-x] [-v] hostname\n",v[0]);
		log(EVDNS_LOG_WARN, "syntax: %s [-servertest]\n", v[0]);
		return 1;
	}
	idx = 1;
	while (idx < c && v[idx][0] == '-') {
		if (!strcmp(v[idx], "-x"))
			...
			log(EVDNS_LOG_WARN, "Unknown option %s\n", v[idx]);
		++idx;
	}
	event_init();
	if (verbose)
	...
	for (; idx < c; ++idx) {
		if (reverse) {
			...
			if (!inet_aton(v[idx], &addr)) {
				log(EVDNS_LOG_WARN, "Skipping non-IP %s\n", v[idx]);
				continue;
			}
			log(EVDNS_LOG_WARN, "resolving %s...\n",v[idx]);
			evdns_resolve_reverse(&addr, 0, main_callback, v[idx]);
		} else {
			log(EVDNS_LOG_WARN, "resolving (fwd) %s...\n",v[idx]);
