...
int configure_nameservers(int force);
...
evdns_log_cb(int warn, const char *msg)
{
  ...
  if (!strcmpstart(msg, "Resolve requested for") &&
    ...
    log(LOG_INFO,LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_REQUESTED));
    return;
  ...
  if (! server_mode(options)) {
    if (!the_evdns_base) {
      if (!(the_evdns_base = evdns_base_new(tor_libevent_get_base(), 0))) {
        log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_NO_EVDNS_BASE));
        return -1;
      }
    }
...
purge_expired_resolves(time_t now)
{
  ...
  while (smartlist_len(cached_resolve_pqueue)) {
    ...
    if (resolve->state == CACHE_STATE_PENDING) {
      char *esc_l = escaped_safe_str(resolve->address);
      log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_TIMEOUT),esc_l);
      tor_free(esc_l);
    } else if (resolve->state == CACHE_STATE_CACHED_VALID ||
               resolve->state == CACHE_STATE_CACHED_FAILED) {
      char *esc_l = escaped_safe_str(resolve->address);
      log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_ENTRY_EXPIRED),esc_l,(unsigned long)resolve->expire);
      tor_free(esc_l);
    ...
    if (resolve->pending_connections) {
      log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_TIMEOUT_2));
      tor_fragile_assert();
    ...
    if (resolve->state == CACHE_STATE_CACHED_VALID ||
      ...
      if (removed != resolve) {
        log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_REMOVED_SOMETHING_ELSE),resolve->address,(void*)resolve,removed ? removed->address : "NULL",(void*)remove);
      }
                 ...
                 or_circuit_t *oncirc, char **hostname_out)
{
  ...
  if (address_is_invalid_destination(exitconn->_base.address, 0)) {
    esc_l = escaped_safe_str(exitconn->_base.address);
    log(LOG_PROTOCOL_WARN, LD_EXIT,get_lang_str(LANG_LOG_DNS_INVALID_ADDRESS),esc_l);
    tor_free(esc_l);
    return -1;
  }
  ...
  if ((r = tor_addr_parse_reverse_lookup_name(&addr, exitconn->_base.address,
    ...
    if (!is_reverse || !is_resolve) {
      if (!is_reverse)
      { esc_l = escaped_safe_str(exitconn->_base.address);
        log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_INVALID_ADDRESS_2),esc_l);
	tor_free(esc_l);
      }
      else if (!is_resolve)
      {	esc_l = escaped_safe_str(exitconn->_base.address);
        log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_INVALID_ADDRESS_3),esc_l);
	tor_free(esc_l);
      }
  ...
  if (resolve && resolve->expire > now) { /* already there */
	...
	esc_l = escaped_safe_str(exitconn->_base.address);
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_PENDING),exitconn->_base.s,esc_l);
	tor_free(esc_l);
        return 0;
      case CACHE_STATE_CACHED_VALID:
        esc_l = escaped_safe_str(resolve->address);
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_CACHED_ANSWER_FOUND),exitconn->_base.s,esc_l);
	tor_free(esc_l);
      ...
      case CACHE_STATE_CACHED_FAILED:
        esc_l = escaped_safe_str(exitconn->_base.address);
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_CACHED_ERROR_FOUND),exitconn->_base.s,esc_l);
	tor_free(esc_l);
        return -1;
      case CACHE_STATE_DONE:
        log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_DONE_STILL_IN_CACHE));
        tor_fragile_assert();
    }
    tor_assert(0);
  }
  ...
  r=get_options()->ResolveTimeout;
  set_expiry(resolve, now + (r?r:30*60));

  esc_l = escaped_safe_str(exitconn->_base.address);
  log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_LAUNCHING),esc_l);
  tor_free(esc_l);
  assert_cache_ok();
...
connection_dns_remove(edge_connection_t *conn)
{
  ...
  resolve = HT_FIND(cache_map, &cache_root, &search);
  if (!resolve) {
    esc_l = escaped_safe_str(conn->_base.address);
    log_notice(LD_BUG,get_lang_str(LANG_LOG_DNS_DROPPING_ADDRESS),esc_l);
    tor_free(esc_l);
    return;
  }
  ...
  if (pend->conn == conn) {
    ...
    tor_free(pend);
    esc_l = escaped_safe_str(conn->_base.address);
    log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_CONNECTION_REMOVED),conn->_base.s,esc_l);
    tor_free(esc_l);
    return;
  } else {
    for ( ; pend->next; pend = pend->next) {
      if (pend->next->conn == conn) {
        ...
        tor_free(victim);
	esc_l = escaped_safe_str(conn->_base.address);
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_CONNECTION_REMOVED_2),conn->_base.s,esc_l);
	tor_free(esc_l);
...
dns_cancel_pending_resolve(const char *address)
{
  ...
  if (resolve->state != CACHE_STATE_PENDING) {
    ...
    if (resolve->pending_connections) {
      ...
      log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_CANCEL_PENDING),esc_l);
      tor_free(esc_l);
      tor_fragile_assert();
    }
    return;
  ...
  if (!resolve->pending_connections) {
    esc_l = escaped_safe_str(address);
    log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_CANCEL_PENDING_2),esc_l);
    tor_free(esc_l);
    tor_fragile_assert();
    return;
  }
  ...
  esc_l = escaped_safe_str(address);
  log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_CANCEL_PENDING_3),esc_l);
  tor_free(esc_l);
  while (resolve->pending_connections) {
    pend = resolve->pending_connections;
  ...
  if (tmp != resolve) {
    log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_CANCEL_PENDING_4),resolve->address, (void*)resolve,tmp ? tmp->address : "NULL", (void*)tmp);
  }
  tor_assert(tmp == resolve);
                    ...
                    const char *hostname, char outcome, uint32_t ttl)
{
  ...
  HT_INSERT(cache_map, &cache_root, resolve);
  set_expiry(resolve, get_time(NULL) + dns_get_expiry_ttl(ttl));
}
                 ...
                 const char *hostname, char outcome, uint32_t ttl)
{
  ...
  if (!resolve) {
    ...
    if (!is_test_addr)
    { esc_l = escaped_safe_str(address);
      log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVED_UNASKED_ADDRESS),esc_l);
      tor_free(esc_l);
    }
  ...
  if (resolve->state != CACHE_STATE_PENDING) {
    ...
    if (!is_test_addr)
    { esc_l = escaped_safe_str(address);
      log_notice(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVED_TWICE),esc_l);
      tor_free(esc_l);
    }
  ...
  while (resolve->pending_connections) {
    ...
    assert_connection_ok(TO_CONN(pendconn),get_time(NULL));
  ...
  if (removed != resolve) {
    log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_ERROR_REMOVING_FROM_CACHE),resolve->address,(void*)resolve,removed ? removed->address : "NULL",(void*)removed);
  }
  assert_resolve_ok(resolve);
  assert_cache_ok();
...
{	or_options_t *options;
	...
	{	tor_addr_t addr;
		...
		{	int socklen;
			...
			socklen = tor_addr_to_sockaddr(&addr,0,(struct sockaddr *)&ss, sizeof(ss));
			if(socklen <= 0)	log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_ERROR_CONVERTING_OUTBOUNDBIND_TO_SOCKADDR));
			else		evdns_base_set_default_outgoing_bind_address(the_evdns_base,(struct sockaddr *)&ss,socklen);
		}
	}
	#endif
	...
	{	if(stat(conf_fname, &st))
			log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVER_CONFIG_ERROR),conf_fname,strerror(errno));
		else if(!force && resolv_conf_fname && !strcmp(conf_fname,resolv_conf_fname) && update_time(st.st_mtime) == resolv_conf_mtime)
		{	log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_NO_CHANGE),conf_fname);
			return 0;
		}
		else
		{	if(nameservers_configured)
			...
			log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVER_CONFIG_PARSE),conf_fname);
			if((r = evdns_base_resolv_conf_parse(the_evdns_base,DNS_OPTIONS_ALL, conf_fname)))
				log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVER_CONFIG_ERROR_2),conf_fname,conf_fname,r);
			else if(evdns_base_count_nameservers(the_evdns_base) == 0)
				log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_NAMESERVERS_NOT_FOUND),conf_fname);
			else
			{	tor_free(resolv_conf_fname);
				resolv_conf_fname = tor_strdup(conf_fname);
				resolv_conf_mtime = update_time(st.st_mtime);
				if(nameservers_configured)	evdns_base_resume(the_evdns_base);
				r = 0;
			}
		}
	}
	...
	{	if(nameservers_configured)
		...
		if(evdns_base_config_windows_nameservers(the_evdns_base))
			log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_NAMESERVERS_CONFIG_ERROR));
		else if(evdns_base_count_nameservers(the_evdns_base) == 0)
			log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_NAMESERVERS_NOT_FOUND_2));
		else
		{	if(nameservers_configured)	evdns_base_resume(the_evdns_base);
			tor_free(resolv_conf_fname);
			resolv_conf_mtime = 0;
	...
	{	if(evdns_base_count_nameservers(the_evdns_base) == 1)
		{	evdns_base_set_option_(the_evdns_base,"max-timeouts:", "16");
			evdns_base_set_option_(the_evdns_base,"timeout:", "10");
		}
		else
		{	evdns_base_set_option_(the_evdns_base,"max-timeouts:", "3");
			evdns_base_set_option_(the_evdns_base,"timeout:", "5");
		}
		if(options->ServerDNSRandomizeCase)
			evdns_base_set_option_(the_evdns_base,"randomize-case:", "1");
		else	evdns_base_set_option_(the_evdns_base,"randomize-case:", "0");
               ...
               void *arg)
{
  ...
  if (result == DNS_ERR_NONE) {
    if (type == DNS_IPv4_A && count) {
      ...
      esc_l = escaped_safe_str(answer_buf);

      if (answer_is_wildcarded(answer_buf)) {
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_ISP_HIJACKED_ADDRESS),safe_str(escaped_address),esc_l);
        was_wildcarded = 1;
        addr = 0;
        status = DNS_RESOLVE_FAILED_PERMANENT;
      } else {
        log_debug(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_OK),safe_str(escaped_address),esc_l);
      }
      ...
      status = DNS_RESOLVE_SUCCEEDED;
      escaped_address = esc_for_log(string_address);
      esc_l = escaped_safe_str(hostname);
      log_debug(LD_EXIT, get_lang_str(LANG_LOG_DNS_RESOLVE_OK),safe_str(escaped_address),esc_l);
      tor_free(escaped_address);
      tor_free(esc_l);
    } else if (count) {
      char *esc_l = escaped_safe_str(string_address);
      log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_NON_IPV4),esc_l);
      tor_free(esc_l);
    } else {
      char *esc_l = escaped_safe_str(string_address);
      log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_RESOLVE_FAILED),esc_l);
      tor_free(esc_l);
    }
  } else {
...
launch_resolve(edge_connection_t *exitconn)
{
  ...
  if (!nameservers_configured) {
    log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_RESOLVE_WITHOUT_CONFIG));
  ...
  if (r == 0) {
    char *esc_l = escaped_safe_str(exitconn->_base.address);
    log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_LAUNCHING_REQUEST),esc_l);
    tor_free(esc_l);
    ...
    char *esc_l = escaped_safe_str(exitconn->_base.address);
    log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_LAUNCHING_REQUEST_2),esc_l);
    tor_free(esc_l);
    ...
    log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_IN_ADDR_ARPA));
  }

  r = 0;
  if (!req) {
    char *esc_l = escaped_safe_str(addr);
    log_warn(LD_EXIT,get_lang_str(LANG_LOG_DNS_ADDRESS_REJECTED),esc_l,r);
    tor_free(esc_l);
    r = -1;
...
wildcard_increment_answer(const char *id)
{
  ...
  if (*ip > 5 && n_wildcard_requests > 10) {
    ...
    log(dns_wildcard_notice_given ? LOG_INFO : LOG_NOTICE,LD_EXIT,get_lang_str(LANG_LOG_DNS_ISP_IS_HIJACKING),id,*ip,id);
...
add_wildcarded_test_address(const char *address)
{
  ...
  if (n > n_test_addrs/2) {
    log(dns_wildcarded_test_address_notice_given ? LOG_INFO : LOG_NOTICE,LD_EXIT,get_lang_str(LANG_LOG_DNS_BROKEN_DNS),address,n);
                              ...
                              void *addresses, void *arg)
{
  ...
  if (result == DNS_ERR_NONE && type == DNS_IPv4_A && count) {
    ...
    log(dns_wildcard_one_notice_given ? LOG_INFO : LOG_NOTICE,LD_EXIT,get_lang_str(LANG_LOG_DNS_ISP_IS_HIJACKING_2),string_address, strmap_size(dns_wildcard_response_count),(strmap_size(dns_wildcard_response_count) == 1) ? get_lang_str(LANG_LOG_DNS__BAD_ADDRESS) : get_lang_str(LANG_LOG_DNS__BAD_ADDRESSES));
...
launch_wildcard_check(int min_len, int max_len, const char *suffix)
{
  ...
  log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_TESTING_DNS),addr);
...
launch_test_addresses(int fd, short event, void *args)
{
  ...
  log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_TESTING_DNS_2));
  /* This situation is worse than the failure-hijacking situation.  When this
   * happens, we're no good for DNS requests at all, and we shouldn't really
   * be an exit server.*/
  if (!options->ServerDNSTestAddresses)
    return;
  SMARTLIST_FOREACH_BEGIN(options->ServerDNSTestAddresses,const char *, address)
  {
    ...
    if (!req) {
      ...
      log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_TEST_ADDRESS_REJECTED),esc_l);
      tor_free(esc_l);
      tor_free(a);
    }
...
{
  int i;
  log_info(LD_EXIT,get_lang_str(LANG_LOG_DNS_TESTING_DNS_3));
...
dns_launch_correctness_checks(void)
{
  ...
  if(!the_evdns_base)
  {	if(has_dns_init_failed())	dns_init();
	else				configure_nameservers(1);
	if(!the_evdns_base)
		return;
  }
  ...
  if (evtimer_add(launch_event, &timeout)<0) {
    log_warn(LD_BUG,get_lang_str(LANG_LOG_DNS_TEST_TIMER_ERROR));
  }
}
...
{
  strmap_free(dns_wildcard_response_count, _tor_free_);
...
dump_dns_mem_usage(int severity)
{
   ...
   */
  log(severity, LD_MM,get_lang_str(LANG_LOG_DNS_MEM_USAGE), hash_count,(unsigned)hash_mem);
}
...
_assert_cache_ok(void)
{
  ...
  if (bad_rep) {
    log_err(LD_BUG,get_lang_str(LANG_LOG_DNS_BAD_REP),bad_rep);
    tor_assert(!bad_rep);
  }
