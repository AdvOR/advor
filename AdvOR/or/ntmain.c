...
typedef   SC_HANDLE (WINAPI *SERVFN1)(
...
typedef   SC_HANDLE (WINAPI *SERVFN2)(
                             LPCTSTR lpMachineName,
                             LPCTSTR lpDatabaseName,
                             DWORD dwDesiredAccess);

typedef   SC_HANDLE (WINAPI *SERVFN3)(
                             SC_HANDLE hSCManager,
                             LPCTSTR lpServiceName,
                             DWORD dwDesiredAccess);

typedef   SERVICE_STATUS_HANDLE (WINAPI *SERVFN4)(
                             LPCTSTR lpServiceName,
...
struct service_fns {
  ...
  SERVFN1 CreateServiceA_fn;

  BOOL (WINAPI *DeleteService_fn)(
                             SC_HANDLE hService);

  SERVFN2 OpenSCManagerA_fn;

  SERVFN3 OpenServiceA_fn;

  BOOL (WINAPI *QueryServiceStatus_fn)(
                             SC_HANDLE hService,
                             LPSERVICE_STATUS lpServiceStatus);

  SERVFN4 RegisterServiceCtrlHandlerA_fn;
...
{	HMODULE library = 0;
	if(service_fns.loaded)	return;
	/* XXXX Possibly, we should hardcode the location of this DLL. */
	if(!(library = LoadLibrary("advapi32.dll")))
		log_err(LD_GENERAL,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_LOAD_ERROR));
	else if(!(service_fns.ChangeServiceConfig2A_fn = GetProcAddress(library,"ChangeServiceConfig2A")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"ChangeServiceConfig2A");
	else if(!(service_fns.CloseServiceHandle_fn = GetProcAddress(library,"CloseServiceHandle")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"CloseServiceHandle");
	else if(!(service_fns.ControlService_fn = GetProcAddress(library,"ControlService")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"ControlService");
	else if(!(service_fns.CreateServiceA_fn = (SERVFN1)GetProcAddress(library,"CreateServiceA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"CreateServiceA");
	else if(!(service_fns.DeleteService_fn = GetProcAddress(library,"DeleteService")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"DeleteService");
	else if(!(service_fns.OpenSCManagerA_fn = (SERVFN2)GetProcAddress(library,"OpenSCManagerA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"OpenSCManagerA");
	else if(!(service_fns.OpenServiceA_fn = (SERVFN3)GetProcAddress(library,"OpenServiceA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"OpenServiceA");
	else if(!(service_fns.QueryServiceStatus_fn = GetProcAddress(library,"QueryServiceStatus")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"QueryServiceStatus");
	else if(!(service_fns.RegisterServiceCtrlHandlerA_fn = (SERVFN4)GetProcAddress(library,"RegisterServiceCtrlHandlerA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"RegisterServiceCtrlHandlerA");
	else if(!(service_fns.SetServiceStatus_fn = GetProcAddress(library,"SetServiceStatus")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"SetServiceStatus");
	else if(!(service_fns.StartServiceCtrlDispatcherA_fn = GetProcAddress(library,"StartServiceCtrlDispatcherA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"StartServiceCtrlDispatcherA");
	else if(!(service_fns.StartServiceA_fn = GetProcAddress(library,"StartServiceA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"StartServiceA");
	else if(!(service_fns.LookupAccountNameA_fn = GetProcAddress(library,"LookupAccountNameA")))
		log_err(LD_BUG,get_lang_str(LANG_LOG_NTMAIN_ADVAPI32_WITHOUT_API),"LookupAccountNameA");
	else
...
nt_service_install(int argc, char **argv)
{
  ...
  if (!strcmp(user_acct,GENSRV_USERACCT)) {
