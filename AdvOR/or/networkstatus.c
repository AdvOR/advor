...
void update_v2_networkstatus_cache_downloads(time_t now);
void update_consensus_networkstatus_downloads(time_t now);
int networkstatus_set_current_consensus1(char *consensus,const char *flavor,unsigned flags);
...
{
  int i;
  const smartlist_t *networkstatus_v2_list2 = networkstatus_get_v2_list();
  SMARTLIST_FOREACH(networkstatus_v2_list2, networkstatus_v2_t *, ns,
...
{	smartlist_t *entries;
	struct stat st;
	char *s;
	char *filename = get_datadir_fname(DATADIR_CACHED_STATUS);
	...
	{	if(maybe_delete)
		{	remove_file_if_very_old(fn, now);
			continue;
		}
		s = read_file_to_str(fn, 0, &st);
		if(s)
		{	if(router_set_networkstatus_v2(s, update_time(st.st_mtime), NS_FROM_CACHE,NULL)<0)
				log_warn(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_LOAD_ERROR),filename);
			tor_free(s);
		}
	});
	SMARTLIST_FOREACH(entries, char *, fn, tor_free(fn));
	smartlist_free(entries);
	networkstatus_v2_list_clean(get_time(NULL));
	routers_update_all_from_networkstatus(get_time(NULL), 2);
	return 0;
}
...
{	char *filename;
	...
	{	char buf[128];
		...
		if(flav == FLAV_NS)
			filename = get_datadir_fname(DATADIR_CACHED_CONSENSUS);
		else
		{	tor_snprintf(buf, sizeof(buf), "%s-%s", DATADIR_CACHED_CONSENSUS,flavor);
			filename = get_datadir_fname(buf);
		}
		s = read_file_to_str(filename, RFTS_IGNORE_MISSING, NULL);
		if(s)
		{	if(networkstatus_set_current_consensus(s, flavor, flags) < -1)
				log_warn(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_LOAD_ERROR_2),filename);
			tor_free(s);
		}
		tor_free(filename);
		if(flav == FLAV_NS)
			filename = get_datadir_fname(DATADIR_UNVERIFIED_CONSENSUS);
		else
		{	tor_snprintf(buf, sizeof(buf), "%s-%s", DATADIR_UNVERIFIED_CONSENSUS,flavor);
			filename = get_datadir_fname(buf);
		}
		s = read_file_to_str(filename, RFTS_IGNORE_MISSING, NULL);
		if(s)
		{	if(networkstatus_set_current_consensus(s,flavor,flags|NSSET_WAS_WAITING_FOR_CERTS))
				log_info(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_LOAD_ERROR_2),filename);
			tor_free(s);
		}
		tor_free(filename);
	}
	if(!current_consensus || (get_file_time(options->FallbackNetworkstatusFile) > current_consensus->valid_after))
	{	s = read_file_to_str(options->FallbackNetworkstatusFile,RFTS_IGNORE_MISSING, NULL);
		if(s)
		{	if(networkstatus_set_current_consensus(s,"ns",flags|NSSET_ACCEPT_OBSOLETE))
				log_info(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_LOAD_ERROR_2),options->FallbackNetworkstatusFile);
			else
				log_notice(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_LOADED),options->FallbackNetworkstatusFile);
			tor_free(s);
		}
	}
	if(!current_consensus)
	{	if(!named_server_map)	named_server_map = strmap_new();
		if(!unnamed_server_map)	unnamed_server_map = strmap_new();
	}
	update_certificate_downloads(get_time(NULL));
	routers_update_all_from_networkstatus(get_time(NULL), 3);
	return 0;
}
...
#ifdef DEBUG_MALLOC
void
routerstatus_free(routerstatus_t *rs,const char *c,int n)
{
  if (!rs)
    return;
  _tor_free_(rs->exitsummary,c,n);
  _tor_free_(rs,c,n);
}
#else
void
                                       ...
                                       const authority_cert_t *cert)
{
  ...
  if (crypto_pk_public_checksig(cert->signing_key,
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_BAD_SIGNATURE));
    sig->bad_signature = 1;
  } else {
                                        ...
                                        int warn)
{
    ...
    SMARTLIST_FOREACH_BEGIN(voter->sigs, document_signature_t *, sig) {
      if (!sig->good_signature && !sig->bad_signature &&
        ...
        if (!is_v3_auth) {
          ...
          ++unknown_here;
          continue;
        } else if (!cert || cert->expires < now-get_options()->MaxTimeDelta) {
  ...
  if (warn > 1 || (warn >= 0 && (n_good + n_missing_key - n_dl_failed_key < n_required)))
    severity = LOG_WARN;
  else
    severity = LOG_INFO;
  ...
  if (warn >= 0) {
    SMARTLIST_FOREACH(unrecognized, networkstatus_voter_info_t *, voter,
      {
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNRECOGNIZED_AUTHORITY),voter->nickname,voter->address,(int)voter->dir_port,voter->contact?voter->contact:"n/a",hex_str(voter->identity_digest,DIGEST_LEN));
      });
    SMARTLIST_FOREACH(need_certs_from, networkstatus_voter_info_t *, voter,
      {
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_NEED_CERT),voter->nickname,voter->address,(int)voter->dir_port,voter->contact?voter->contact:"n/a",hex_str(voter->identity_digest, DIGEST_LEN));
      });
    SMARTLIST_FOREACH(missing_authorities, trusted_dir_server_t *, ds,
      {
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_CONSENSUS_WITHOUT_AUTHORITY),ds->nickname,ds->address,(int)ds->dir_port,hex_str(ds->v3_identity_digest, DIGEST_LEN));
      });

    {
      ...
      tor_asprintf(&cp,get_lang_str(LANG_LOG_NETWORKSTATUS_BAD_SIGNATURE_2),n_required, n_good,tmp);
      tor_free(tmp);
      smartlist_add(sl,cp);
      if (n_no_signature) {
        tmp = smartlist_join_strings(list_no_signature, " ", 0, NULL);
        tor_asprintf(&cp,get_lang_str(LANG_LOG_NETWORKSTATUS_UNVERIFIABLE_SIGNATURE),n_no_signature,tmp);
        tor_free(tmp);
        smartlist_add(sl,cp);
      }
      if (n_unknown) {
        tor_asprintf(&cp,get_lang_str(LANG_LOG_NETWORKSTATUS_UNRECOGNIZED_AUTHORITIES),n_unknown);
        smartlist_add(sl,cp);
      }
      if (n_bad) {
        tor_asprintf(&cp,get_lang_str(LANG_LOG_NETWORKSTATUS_INVALID_SIGNATURE_2),n_bad);
        smartlist_add(sl,cp);
      }
      if (n_missing_key) {
...
//    log(severity, LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_STATS),n_unknown,n_missing_key,n_good,n_bad,n_no_signature,n_required);
  }
...
networkstatus_get_cache_filename(const char *identity_digest)
{
  ...
  return get_datadir_fname2_suffix(DATADIR_CACHED_STATUS,fp,NULL);
}
 ...
 * appropriate. */
static int
add_networkstatus_to_cache(char *s,
                           v2_networkstatus_source_t source,
                           networkstatus_v2_t *ns)
{
  if (source != NS_FROM_CACHE) {
    char *fn = networkstatus_get_cache_filename(ns->identity_digest);
    if (write_buf_to_file(fn,s,strlen(s))<0) {
      log_notice(LD_FS,get_lang_str(LANG_LOG_NETWORKSTATUS_ERROR_WRITING_NETWORKSTATUS),fn);
    }
    tor_free(fn);
  }
 ...
 */
int
router_set_networkstatus_v2(char *s, time_t arrived_at,
                            v2_networkstatus_source_t source,
                            smartlist_t *requested_fingerprints)
{
  ...
  if (!ns) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_ERROR_PARSING_NETWORKSTATUS));
    return -1;
  }
  ...
  if (!(trusted_dir =
    ...
    log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_INVALID_SIGNATURE));
    source_desc = fp;
  } else {
  ...
  if (ns->published_on > now + NETWORKSTATUS_ALLOW_SKEW) {
    ...
    log_warn(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_TIME_SKEW),source_desc,dbuf,published);
  ...
  if (requested_fingerprints) {
    if (smartlist_string_isin(requested_fingerprints, fp)) {
      ...
      if (source != NS_FROM_DIR_ALL) {
        ...
        log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNREQUESTED_FINGERPRINT),fp,requested);
        tor_free(requested);
        return 0;
      }
    }
  ...
  if (!trusted_dir) {
    if (!skewed) {
      ...
      log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNRECOGNIZED_AUTHORITY_2),fp);
  ...
  for (i=0; i < smartlist_len(networkstatus_v2_list); ++i) {
    ...
    if (tor_memeq(old_ns->identity_digest, ns->identity_digest, DIGEST_LEN)) {
      if (tor_memeq(old_ns->networkstatus_digest,
        ...
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_ALREADY_HAVE),trusted_dir->description, published);
        ...
        tor_assert(trusted_dir);
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_ALREADY_HAVE_2),trusted_dir->description,published,old_published);
        networkstatus_v2_free(ns);
        download_status_failed(&trusted_dir->v2_ns_dl_status, 0);
        return 0;
      } else {
...
#define V2_NETWORKSTATUS_ROUTER_LIFETIME (3*60*60)
  ...
  log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_SETTING_NEW),source == NS_FROM_CACHE?get_lang_str(LANG_LOG_NETWORKSTATUS__CACHED_FROM):((source == NS_FROM_DIR_BY_FP || source == NS_FROM_DIR_ALL) ?get_lang_str(LANG_LOG_NETWORKSTATUS__DOWNLOADED_FROM):get_lang_str(LANG_LOG_NETWORKSTATUS__GENERATED_FOR)),trusted_dir->description, published);
...
{	int i;
	or_options_t *options = get_options();
	if((!networkstatus_v2_list)||(options->DirFlags&DIR_FLAG_NO_AUTO_UPDATE))
		return;

	for(i = 0; i < smartlist_len(networkstatus_v2_list); ++i)
	{	networkstatus_v2_t *ns = smartlist_get(networkstatus_v2_list, i);
		char *fname = NULL;
		if((ns->published_on + MAX_NETWORKSTATUS_AGE > now)||(options->DirFlags&DIR_FLAG_NO_AUTO_UPDATE))
			continue;
		/* Okay, this one is too old.  Remove it from the list, and delete it from the cache. */
		smartlist_del(networkstatus_v2_list, i--);
		fname = networkstatus_get_cache_filename(ns->identity_digest);
		if(delete_file(fname))	log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_TOO_OLD),fname);
		tor_free(fname);
		if(directory_caches_v2_dir_info(get_options()))
			dirserv_set_cached_networkstatus_v2(NULL, ns->identity_digest, 0);
		networkstatus_v2_free(ns);
	}
	/* And now go through the directory cache for any cached untrusted networkstatuses and other network info. */
                                        ...
                                        int warn_if_unnamed)
{
  ...
  if (smartlist_len(matches)>1 && warn_if_unnamed) {
    ...
    if (any_unwarned) {
      log_warn(LD_CONFIG,get_lang_str(LANG_LOG_NETWORKSTATUS_MULTIPLE_MATCHES_FOR_NICKNAME),nickname);
    }
  } else if (warn_if_unnamed && best && !best->name_lookup_warned) {
    char fp[HEX_DIGEST_LEN+1];
    base16_encode(fp, sizeof(fp),
                  best->identity_digest, DIGEST_LEN);
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_NETWORKSTATUS_EXIT_TO_NICK),nickname,fp);
...
//  else if(get_options()->DirFlags&DIR_FLAG_NO_AUTO_UPDATE) return;
  if (time_to_download_next_consensus > now)
    return; /* Wait until the current consensus is older. */
  if (!download_status_is_ready(&consensus_dl_status[FLAV_NS], now,
                                get_options()->MaxDlFailures?get_options()->MaxDlFailures:32767) && (bootstrap_percent>=80 || get_options()->DirProxy || we_are_hibernating()))
  ...
  for (i=0; i < N_CONSENSUS_FLAVORS; ++i) {
    ...
    if (waiting->consensus) {
      ...
      if (waiting->set_at + DELAY_WHILE_FETCHING_CERTS > now && (bootstrap_percent>=80 || get_options()->DirProxy || we_are_hibernating()))
  ...
  log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_DOWNLOADING));
...
update_consensus_networkstatus_fetch_time(time_t now)
{
  ...
  if (c) {
    ...
    if(options->DirFlags&DIR_FLAG_NO_AUTO_UPDATE) time_to_download_next_consensus = -1;
    else
    {
      ...
      log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_HAVE_LIVE_CONSENSUS),(c->fresh_until > now) ? get_lang_str(LANG_LOG_NETWORKSTATUS__LC_WILL_BE) : get_lang_str(LANG_LOG_NETWORKSTATUS__LC_WAS),tbuf1,tbuf2,tbuf3);
    }
  } else {
    time_to_download_next_consensus = now;
    log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_NO_LIVE_CONSENSUS));
  }
...
{
  if ((options->UseBridges && !any_bridge_descriptors_known()) || we_are_hibernating()) {
    log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_DIR_FETCHES_DELAYED));
    return 1;
  }
  return 0;
}
...
{
  or_options_t *options = get_options();
  if (should_delay_dir_fetches(options)||((options->DirFlags&DIR_FLAG_NO_AUTO_UPDATE)&&current_consensus))
    return;
...
{
  return consensus_waiting_for_certs[USABLE_CONSENSUS_FLAVOR].consensus ? 1 : 0;
}
...
{
  if (current_consensus && (((current_consensus->valid_after <= now) && (now <= current_consensus->valid_until))||(get_options()->DirFlags&DIR_FLAG_NO_AUTO_UPDATE)))
    return current_consensus;
  else
    return NULL;
}
...
#define REASONABLY_LIVE_TIME (24*60*60)
  if (current_consensus && (((current_consensus->valid_after <= now) && (now <= current_consensus->valid_until+REASONABLY_LIVE_TIME))||(get_options()->DirFlags&DIR_FLAG_NO_AUTO_UPDATE)))
    return current_consensus;
  else
    return NULL;
}
 ...
 */
int networkstatus_set_current_consensus(char *consensus,const char *flavor,unsigned flags)
{	networkstatus_t *c=NULL;
	...
	if(flav < 0)	/* XXXX we don't handle unrecognized flavors yet. */
	{	log_warn(LD_BUG,get_lang_str(LANG_LOG_NETWORKSTATUS_UNRECOGNIZED_FLAVOR),flavor);
		return -2;
	}
	c = networkstatus_parse_vote_from_string(consensus,NULL,NS_TYPE_CONSENSUS);	/* Make sure it's parseable. */
	if(!c)
	{	log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_PARSE_ERROR));
		return -2;
	}
	if((int)c->flavor != flav)	/* This wasn't the flavor we thought we were getting. */
	{	if(require_flavor)
		{	log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNEXPECTED_FLAVOR),networkstatus_get_flavor_name(c->flavor),flavor);
	...
	if(flav != USABLE_CONSENSUS_FLAVOR && !directory_caches_dir_info(options))	/* This consensus is totally boring to us: we won't use it, and we won't serve it. Drop it. */
	{	networkstatus_vote_free(c);
		return -1;
	}
	if(from_cache && !accept_obsolete && c->valid_until < now-OLD_ROUTER_DESC_MAX_AGE)	/* XXXX If we try to make fallbackconsensus work again, we should consider taking this out. Until then, believing obsolete consensuses is causing more harm than good. See also bug 887. */
	{	log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_OBSOLETE));
		networkstatus_vote_free(c);
		return -1;
	}
	if(!strcmp(flavor, "ns"))
	{	consensus_fname = get_datadir_fname(DATADIR_CACHED_CONSENSUS);
		unverified_fname = get_datadir_fname(DATADIR_UNVERIFIED_CONSENSUS);
		if(current_consensus)
	...
	if(current_digests && tor_memeq(&c->digests, current_digests, sizeof(c->digests)))	/* We already have this one. That's a failure. */
	{	log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_ALREADY_HAVE_3));
		networkstatus_vote_free(c);
		return -1;
	}
	else if(current_valid_after && c->valid_after <= current_valid_after)			/* We have a newer one. There's no point in accepting this one, even if it's great. */
	{	log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_ALREADY_HAVE_4));
		networkstatus_vote_free(c);
		return -1;
	}
	if((r=networkstatus_check_consensus_signature(c, 1))<0)		/* Make sure it's signed enough. */
	{	if(r == -1)	/* Okay, so it _might_ be signed enough if we get more certificates. */
		{	if(!was_waiting_for_certs)
				log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_NOT_ENOUGH_CERTS));
			...
			{	waiting = &consensus_waiting_for_certs[flav];
				...
				waiting->set_at = now;
				waiting->dl_failed = 0;
				if(!from_cache)		write_buf_to_file(unverified_fname,consensus,strlen(consensus));
				if(dl_certs)		authority_certs_fetch_missing(c,now);
				/* This case is not a success or a failure until we get the certs or fail to get the certs. */
				result = 0;
			}
			else	/* Even if we had enough signatures, we'd never use this as the latest consensus. */
			{	if(was_waiting_for_certs && from_cache)	delete_file(unverified_fname);
				networkstatus_vote_free(c);
			}
		}
		else	/* This can never be signed enough:  Kill it. */
		{	if(!was_waiting_for_certs)
			{	log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_NOT_ENOUGH_SIGNATURES));
				result = -2;
			}
			if(was_waiting_for_certs && (r < -1) && from_cache)	delete_file(unverified_fname);
			networkstatus_vote_free(c);
		}
	...
	if(!from_cache && flav == USABLE_CONSENSUS_FLAVOR)	control_event_client_status(LOG_NOTICE, "CONSENSUS_ARRIVED");
	if(r != 1 && dl_certs)	authority_certs_fetch_missing(c, now);	/* Are we missing any certificates at all? */
	if(flav == USABLE_CONSENSUS_FLAVOR)
	{	notify_control_networkstatus_changed(current_consensus, c);
		if(current_consensus)
		{	networkstatus_copy_old_consensus_info(c,current_consensus);
			networkstatus_vote_free(current_consensus);
			current_consensus = NULL;	/* Defensive programming : we should set current_consensus very soon, but we're about to call some stuff in the meantime, and leaving this dangling pointer around has proven to be trouble. */
		}
	}
	waiting = &consensus_waiting_for_certs[flav];
	if(waiting->consensus && waiting->consensus->valid_after <= c->valid_after)
	{	networkstatus_vote_free(waiting->consensus);
		waiting->consensus = NULL;
		if(consensus != waiting->body)	tor_free(waiting->body);
		else				waiting->body = NULL;
		waiting->set_at = 0;
		waiting->dl_failed = 0;
		delete_file(unverified_fname);
	}
	/* Reset the failure count only if this consensus is actually valid. */
	if(c->valid_after <= now && now <= c->valid_until)
		download_status_reset(&consensus_dl_status[flav]);
	else if(!from_cache)	download_status_failed(&consensus_dl_status[flav], 0);
	if(flav == USABLE_CONSENSUS_FLAVOR)
	{	current_consensus = c;
		free_consensus = 0; /* Prevent free. */
		update_consensus_networkstatus_fetch_time(now);	/* XXXXNM Microdescs: needs a non-ns variant. */
		dirvote_recalculate_timing(options, now);
		routerstatus_list_update_named_server_map();
		cell_ewma_set_scale_factor(options, current_consensus);
		connection_or_update_token_buckets(get_connection_array(), options);	/* XXXX023 this call might be unnecessary here: can changing the current consensus really alter our view of any OR's rate limits? */
		circuit_build_times_new_consensus_params(&circ_times, current_consensus);
	}
	if(directory_caches_dir_info(options))
		dirserv_set_cached_consensus_networkstatus(consensus,flavor,&c->digests,c->valid_after);
	if(!from_cache)
		write_buf_to_file(consensus_fname,consensus,strlen(consensus));
	/** If a consensus appears more than this many seconds before its declared valid-after time, declare that our clock is skewed. */
	#define EARLY_CONSENSUS_NOTICE_SKEW 60
	...
	{	char tbuf[ISO_TIME_LEN+1];
		...
		format_time_interval(dbuf, sizeof(dbuf), delta);
		log_warn(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_TIME_SKEW_2),dbuf,tbuf);
		control_event_general_status(LOG_WARN,"CLOCK_SKEW MIN_SKEW=%ld SOURCE=CONSENSUS", delta);
	}
	router_dir_info_changed();
	if(free_consensus)	networkstatus_vote_free(c);
	tor_free(consensus_fname);
	tor_free(unverified_fname);
	return 0;
}
...
{	networkstatus_t *c=NULL;
	...
	const unsigned was_waiting_for_certs = flags & NSSET_WAS_WAITING_FOR_CERTS;
	const unsigned dl_certs = !(flags & NSSET_DONT_DOWNLOAD_CERTS);
	const unsigned accept_obsolete = 1;//flags & NSSET_ACCEPT_OBSOLETE;
	const unsigned require_flavor = flags & NSSET_REQUIRE_FLAVOR;
	const digests_t *current_digests = NULL;
	consensus_waiting_for_certs_t *waiting = NULL;
	time_t current_valid_after = 0;
	...
	if(flav != USABLE_CONSENSUS_FLAVOR && !directory_caches_dir_info(options))	/* This consensus is totally boring to us: we won't use it, and we won't serve it.  Drop it. */
		;
	else if(from_cache && !accept_obsolete &&c->valid_until < now-OLD_ROUTER_DESC_MAX_AGE)
	{	/* XXX022 when we try to make fallbackconsensus work again, we should consider taking this out. Until then, believing obsolete consensuses is causing more harm than good. See also bug 887. */
		log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_OBSOLETE));
	}
	else
	{	if(!strcmp(flavor,"ns"))
		...
		{	if((r=networkstatus_check_consensus_signature(c, 1))<0)				/* Make sure it's signed enough. */
			{	if(r == -1)								/* Okay, so it _might_ be signed enough if we get more certificates. */
				{	if(!was_waiting_for_certs)	log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_NOT_ENOUGH_CERTS));
					if(!current_valid_after || c->valid_after >= current_valid_after)
					{	waiting = &consensus_waiting_for_certs[flav];
						networkstatus_vote_free(waiting->consensus);
						tor_free(waiting->body);
			...
			{	if(!from_cache && flav == USABLE_CONSENSUS_FLAVOR)	control_event_client_status(LOG_NOTICE, "CONSENSUS_ARRIVED");
				...
				if(r != 1 && dl_certs)	authority_certs_fetch_missing(c, now);
				if(flav == USABLE_CONSENSUS_FLAVOR)
				{	notify_control_networkstatus_changed(current_consensus, c);
					if(current_consensus)
					{	networkstatus_copy_old_consensus_info(c,current_consensus);
				...
				{	current_consensus = c;
					...
					routerstatus_list_update_named_server_map();
					cell_ewma_set_scale_factor(options, current_consensus);
					/* XXXX023 this call might be unnecessary here: can changing the current consensus really alter our view of any OR's rate limits? */
				...
				if(c->valid_after <= now && now <= c->valid_until)		/* Reset the failure count only if this consensus is actually valid. */
					download_status_reset(&consensus_dl_status[flav]);
				else if(!from_cache)	download_status_failed(&consensus_dl_status[flav], 0);
				if(flav == USABLE_CONSENSUS_FLAVOR)
				{	current_consensus = c;
				...
				if(directory_caches_dir_info(get_options()))
					dirserv_set_cached_consensus_networkstatus(consensus,flavor,&c->digests,c->valid_after);
				if(!from_cache)	write_buf_to_file(consensus_fname,consensus,strlen(consensus));
				if(get_options()->DirFlags&DIR_FLAG_FAKE_LOCAL_TIME)
				{	if((now - current_consensus->valid_after)<0)
						update_best_delta_t(best_delta_t + now - current_consensus->valid_after + crypto_rand_int(20*60));
				}
				else if(now < current_consensus->valid_after - EARLY_CONSENSUS_NOTICE_SKEW)
				{	char tbuf[ISO_TIME_LEN+1];
					char dbuf[64];
					long delta = now - current_consensus->valid_after;
					format_iso_time(tbuf, current_consensus->valid_after);
...
routers_update_all_from_networkstatus(time_t now, int dir_version)
{
  ...
  if (!have_warned_about_old_version) {
    ...
    if (status == VS_RECOMMENDED) {
      log_info(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_VERSION_OK));
    } else if (status == VS_EMPTY) {
      log_info(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_NO_VERSIONS));
    } else if (status == VS_NEW || status == VS_NEW_IN_SERIES) {
      if (!have_warned_about_new_version) {
        log_notice(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_VERSION_TOO_NEW),
#ifdef int3
                                     get_options()->torver,
#else
                                     VERSION,
...
#ifdef int3
                                     get_options()->torver, "NEW", recommended);
#else
                                     VERSION, "NEW", recommended);
#endif
      }
    } else {
      log_warn(LD_GENERAL,get_lang_str(LANG_LOG_NETWORKSTATUS_VERSION_TOO_OLD),
#ifdef int3
                                     get_options()->torver,
#else
                                     VERSION,
#endif
               status == VS_OLD ? get_lang_str(LANG_LOG_NETWORKSTATUS__OBSOLETE) : get_lang_str(LANG_LOG_NETWORKSTATUS__NOT_RECOMMENDED),
               recommended);
      ...
      control_event_general_status(LOG_WARN, "DANGEROUS_VERSION "
           "CURRENT=%s REASON=%s RECOMMENDED=\"%s\"",
                                     ...
                                     VERSION,
#endif
           status == VS_OLD ? "OBSOLETE" : "UNRECOMMENDED",
           recommended);
    }
  }
}
...
download_status_map_update_from_v2_networkstatus(void)
{
  ...
  digestmap_free(v2_download_status_map, _tor_free_);
...
routerstatus_list_update_named_server_map(void)
{
  ...
  if (named_server_map)
    strmap_free(named_server_map, _tor_free_);
                                                   ...
                                                   int reset_failures)
{
  ...
  SMARTLIST_FOREACH(networkstatus_v2_list, networkstatus_v2_t *, ns2, {
    time_t live_until = ns2->published_on + V2_NETWORKSTATUS_ROUTER_LIFETIME;
    SMARTLIST_FOREACH_JOIN(ns2->entries, routerstatus_t *, rs,
...
networkstatus_getinfo_by_purpose(const char *purpose_string, time_t now)
{
  ...
  if (purpose == ROUTER_PURPOSE_UNKNOWN) {
    log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNRECOGNIZED_PURPOSE),purpose_string);
    return NULL;
  }
...
networkstatus_dump_bridge_status_to_file(time_t now)
{
  ...
  char *fname = get_datadir_fname(DATADIR_NETWORKSTATUS_BRIDGES);
  write_buf_to_file(fname,status,strlen(status));
  tor_free(fname);
  tor_free(status);
}
                        ...
                        int32_t default_val, int32_t min_val, int32_t max_val)
{
  ...
  if (res < min_val) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_PARAMETER_TOO_SMALL),param_name,res,min_val);
    res = min_val;
  } else if (res > max_val) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_PARAMETER_TOO_LARGE),param_name,res,max_val);
    res = max_val;
  }
                            ...
                            int32_t default_val)
{
  ...
  param = get_net_param_from_list(ns->weight_params, weight_name,
                                  ...
                                  BW_MAX_WEIGHT_SCALE);
  if (param > max) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_WEIGHT_TOO_LARGE),weight_name,max);
    param = max;
  }
  return param;
}
                             ...
                             const char **errmsg)
{
  ...
  if (!strcmp(question, "ns/all")) {
    ...
    *answer = networkstatus_getinfo_by_purpose(question+11, get_time(NULL));
    return *answer ? 0 : -1;
  } else {
    return 0;
  }
...
void expire_consensus(void)
{	if(current_consensus)
	{	current_consensus->valid_after=1147483647L;
		current_consensus->valid_until=0;
	}
