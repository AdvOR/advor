...
connection_or_remove_from_identity_map(or_connection_t *conn)
{
  ...
  if (!tmp) {
    if (!tor_digest_is_zero(conn->identity_digest)) {
      log_warn(LD_BUG,get_lang_str(LANG_LOG_CONN_OR_ERROR_REMOVING_DIGEST),conn->nickname ? conn->nickname : "NULL");
    }
    return;
  }
  if (conn == tmp) {
...
{
  log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_EOF));
...
#define MAX_OR_INBUF_WHEN_NONOPEN 0
  ...
  if (buf_datalen(conn->_base.inbuf) > MAX_OR_INBUF_WHEN_NONOPEN) {
	...
	log_fn(LOG_PROTOCOL_WARN,LD_NET,get_lang_str(LANG_LOG_CONNECTION_OR_TOO_MUCH_DATA_1),(int)buf_datalen(conn->_base.inbuf),conn->_base.address, conn->_base.port,conn_state_to_string(conn->_base.type, conn->_base.state));
    else
	log_fn(LOG_PROTOCOL_WARN,LD_NET,get_lang_str(LANG_LOG_CONNECTION_OR_TOO_MUCH_DATA_2),(int)buf_datalen(conn->_base.inbuf),connection_or_nonopen_was_started_here(conn) ? "to" : "from",conn->_base.address, conn->_base.port,conn_state_to_string(conn->_base.type, conn->_base.state));
    connection_mark_for_close(TO_CONN(conn));
    ret = -1;
  }
}
...
connection_or_finished_flushing(or_connection_t *conn)
{
  ...
  switch (conn->_base.state) {
    ...
    case OR_CONN_STATE_OR_HANDSHAKING:
      ...
      break;
    default:
      log_err(LD_BUG,get_lang_str(LANG_LOG_CONN_OR_FINISHED_FLUSHING_UNEXPECTED),conn->_base.state);
      tor_fragile_assert();
      return -1;
  }
  return 0;
...
{	connection_t *conn;
	...
	log_debug(LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_FINISHED_CONNECTING),conn->address,conn->port);
	control_event_bootstrap(BOOTSTRAP_STATUS_HANDSHAKE, 0);
	if((get_options()->DirFlags&DIR_FLAG_NTLM_PROXY) && get_options()->CorporateProxy)
	{	if(connection_proxy_connect(conn,1) < 0)
	...
	else if((get_options()->DirFlags&DIR_FLAG_HTTPS_PROXY) && get_options()->ORProxy)
	{	if(connection_proxy_connect(conn,0) < 0)
		{	connection_mark_for_close(conn);
			return -1;
		}
		connection_start_reading(conn);
	...
	else if(connection_tls_start_handshake(or_conn, 0) < 0)	/* TLS handshaking error of some kind. */
	{	connection_mark_for_close(conn);
		return -1;
	}
	return 0;
...
{	if(router_get_consensus_status_by_id(id_digest))	return 1;	/* It's in the consensus: "yes" */
	if(router_get_by_digest(id_digest))			return 1;	/* Not in the consensus, but we have a descriptor for it. Probably it was in a recent consensus. "Yes". */
	return 0;
}
 ...
 * If <b>reset</b> is true, set the bucket to be full. Otherwise, just clip the bucket if it happens to be <em>too</em> full. */
static void connection_or_update_token_buckets_helper(or_connection_t *conn, int reset,or_options_t *options)
{	int rate, burst; /* per-connection rate limiting params */
	if(connection_or_digest_is_known_relay(conn->identity_digest))
	{	/* It's in the consensus, or we have a descriptor for it meaning it was probably in a recent consensus. It's a recognized relay: give it full bandwidth. */
		rate = (int)options->BandwidthRate;
		burst = (int)options->BandwidthBurst;
	}
	else	/* Not a recognized relay. Squeeze it down based on the suggested bandwidth parameters in the consensus, but allow local config options to override. */
	{	rate = options->PerConnBWRate ? (int)options->PerConnBWRate : networkstatus_get_param(NULL, "perconnbwrate",(int)options->BandwidthRate, 1, INT32_MAX);
		burst = options->PerConnBWBurst ? (int)options->PerConnBWBurst : networkstatus_get_param(NULL, "perconnbwburst",(int)options->BandwidthBurst, 1, INT32_MAX);
	}
	...
	/* If the new token bucket is smaller, take out the extra tokens. (If it's larger, don't -- the buckets can grow to reach the cap.) */
	if(conn->read_bucket > burst)	conn->read_bucket = burst;
	if(conn->write_bucket > burst)	conn->write_bucket = burst;
}
...
void connection_or_update_token_buckets(smartlist_t *conns, or_options_t *options)
{	SMARTLIST_FOREACH(conns, connection_t *, conn,
...
connection_or_group_set_badness(or_connection_t *head, int force)
{
  ...
  for (or_conn = head; or_conn; or_conn = or_conn->next_with_same_id) {
    ...
    if (force ||
        ...
        < now) {
      log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONN_TOO_OLD),or_conn->_base.address,or_conn->_base.port,or_conn->_base.s,(int)(now - or_conn->_base.timestamp_created));
  ...
  for (or_conn = head; or_conn; or_conn = or_conn->next_with_same_id) {
    ...
    if (n_canonical && !or_conn->is_canonical) {
      ...
      log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONN_TOO_OLD_2),or_conn->_base.address, or_conn->_base.port, or_conn->_base.s,(int)(now - or_conn->_base.timestamp_created));
      or_conn->is_bad_for_new_circs = 1;
      continue;
    }
  ...
  for (or_conn = head; or_conn; or_conn = or_conn->next_with_same_id) {
    ...
    if (or_conn != best && connection_or_is_better(now, best, or_conn, 1)) {
      ...
      if (best->is_canonical) {
        log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONN_TOO_OLD_3),or_conn->_base.address,or_conn->_base.port, or_conn->_base.s,(int)(now - or_conn->_base.timestamp_created),best->_base.s,(int)(now - best->_base.timestamp_created));
        or_conn->is_bad_for_new_circs = 1;
      } else if (!tor_addr_compare(&or_conn->real_addr,
                                   &best->real_addr, CMP_EXACT)) {
        log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONN_TOO_OLD_4),or_conn->_base.address,or_conn->_base.port,or_conn->_base.s,(int)(now - or_conn->_base.timestamp_created),best->_base.s,(int)(now - best->_base.timestamp_created));
        or_conn->is_bad_for_new_circs = 1;
      }
    }
  }
                      ...
                      const char *id_digest)
{
  ...
  if (server_mode(options) && router_digest_is_me(id_digest)) {
    log_info(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_REFUSING_SELF_CONN));
    return NULL;
  }
  ...
  if(options->DirFlags&DIR_FLAG_NTLM_PROXY && options->CorporateProxy)
  {
    ...
    port = options->CorporateProxyPort;
  }
  else if(options->DirFlags&DIR_FLAG_HTTPS_PROXY && options->ORProxy)
  {
    using_proxy = 1;
    tor_addr_copy(&addr, &options->ORProxyAddr);
...
connection_tls_start_handshake(or_connection_t *conn, int receiving)
{
  ...
  log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_TLS_HANDSHAKE_START), conn->_base.s);
...
{	int result = TOR_TLS_DONE;
	check_no_tls_errors();
	while(result==TOR_TLS_DONE)
	{	if(conn->_base.state == OR_CONN_STATE_TLS_CLIENT_RENEGOTIATING)
		...
		{	if(!tor_tls_used_v1_handshake(conn->tls))
			{	if(tor_tls_is_server(conn->tls))	/* improved handshake, but not a client. */
				{	log_debug(LD_OR,get_lang_str(LANG_LOG_CONNECTION_OR_RENEGOTIATE_2));
					tor_tls_set_renegotiate_callback(conn->tls,connection_or_tls_renegotiated_cb,conn);
				...
				{	log_debug(LD_OR,get_lang_str(LANG_LOG_CONNECTION_OR_RENEGOTIATE_1));
	...
	{	CASE_TOR_TLS_ERROR_ANY:
			log_info(LD_OR,get_lang_str(LANG_LOG_CONNECTION_TLS_ERROR_3),tor_tls_err_to_string(result));
			return -1;
		case TOR_TLS_WANTWRITE:
			connection_start_writing(TO_CONN(conn));
			log_debug(LD_OR,get_lang_str(LANG_LOG_CONNECTION_WANTED_WRITE));
			return 0;
		case TOR_TLS_WANTREAD: /* handshaking conns are *always* reading */
			log_debug(LD_OR,get_lang_str(LANG_LOG_CONNECTION_WANTED_READ));
			return 0;
		case TOR_TLS_CLOSE:
			log_info(LD_OR,get_lang_str(LANG_LOG_CONNECTION_TLS_CLOSED));
			return -1;
	}
	return 0;
}
                                        ...
                                        char *digest_rcvd_out)
{
  ...
  const char *safe_address =
    started_here ? conn->_base.address : safe_str_client(conn->_base.address);
  ...
  if (started_here && !has_cert) {
    log_info(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_NO_CERT_FROM_ROUTER),safe_address,conn->_base.port);
    return -1;
  } else if (!has_cert) {
    log_debug(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_NO_CERT_FROM_CONN));
  }
  check_no_tls_errors();
  ...
  if (has_cert) {
    ...
    if (started_here && v<0) {
      log_fn(severity,LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_INVALID_CERT_FROM_ROUTER),safe_address,conn->_base.port);
        return -1;
    } else if (v<0) {
      log_info(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_INVALID_CERT_FROM_CONN));
    } else {
      log_debug(LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_CERT_OK),conn_type,safe_address,conn->_base.port);
    }
    check_no_tls_errors();
  }
  ...
  if (started_here && tor_digest_is_zero(conn->identity_digest)) {
    ...
    base16_encode(conn->nickname+1, HEX_DIGEST_LEN+1,
                  conn->identity_digest, DIGEST_LEN);
    log_info(LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_CONNECTED_WITHOUT_KEY),conn->nickname,conn->_base.address,conn->_base.port);
  ...
  if (started_here) {
    ...
    if (tor_memneq(digest_rcvd_out, conn->identity_digest, DIGEST_LEN)) {
      ...
      base16_encode(expected, sizeof(expected), conn->identity_digest,
                    DIGEST_LEN);
      log_fn(severity, LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_UNEXPECTED_KEY_FROM_ROUTER),conn->_base.address,conn->_base.port,expected,seen);
...
connection_tls_finish_handshake(or_connection_t *conn)
{
  ...
  log_debug(LD_HANDSHAKE,get_lang_str(LANG_LOG_CONN_OR_TLS_HANDSHAKE_DONE),safe_str_client(conn->_base.address));
...
connection_or_set_state_open(or_connection_t *conn)
{
  ...
  if (started_here) {
    ...
    if (entry_guard_register_connect_status(conn->identity_digest,
      ...
      log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_RETRY_EARLIER_ENTRYGUARDS));
                                    ...
                                    or_connection_t *conn)
{
  ...
  connection_write_to_buf((char*)cell->payload, cell->payload_len, TO_CONN(conn));
  if (cell->command != CELL_PADDING)
    conn->timestamp_last_added_nonpadding = approx_time();
}
...
connection_or_process_cells_from_inbuf(or_connection_t *conn)
{
  ...
  while (1) {
    log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_PROCESS_CELLS_FROM_INBUF),conn->_base.s,(int)buf_datalen(conn->_base.inbuf),tor_tls_get_pending_bytes(conn->tls));
...
connection_or_send_destroy(circid_t circ_id, or_connection_t *conn, int reason)
{
  ...
  log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_SENDING_DESTROY),circ_id);
...
connection_or_send_versions(or_connection_t *conn)
{
  ...
  conn->handshake_state->sent_versions_at = get_time(NULL);
