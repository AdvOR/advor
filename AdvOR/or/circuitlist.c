...
int info_in_cpath(extend_info_t *info,origin_circuit_t *circ);
...
circuit_count_pending_on_or_conn(or_connection_t *or_conn)
{
  ...
  smartlist_free(sl);
  log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITLIST_OR_CONN_PENDING),or_conn->nickname ? or_conn->nickname : "NULL", cnt);
  return cnt;
}
...
circuit_state_to_string(int state)
{
  ...
  switch (state) {
    ...
    default:
      log_warn(LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_UNKNOWN_CIRCUIT_STATE),state);
...
{
  int32_t num = networkstatus_get_param(NULL, "circwindow", CIRCWINDOW_START,CIRCWINDOW_START_MIN,CIRCWINDOW_START_MAX);
...
void circuit_free(circuit_t *circ)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    if (get_options()->CellStatistics)
      rep_hist_buffer_stats_add_circ(circ, get_time(NULL));
    mem = ocirc;
...
circuit_free_all(void)
{
  ...
  while (global_circuitlist) {
    ...
    if (! CIRCUIT_IS_ORIGIN(global_circuitlist)) {
      ...
      tree_remove_streams(global_circuitlist);
...
void circuit_free_cpath_node(crypt_path_t *victim)
{
  ...
  if(victim->hItem)	tree_remove_hop(victim);
...
{
  log(severity, LD_CIRC,get_lang_str(LANG_LOG_CIRCUITLIST_DUMP_DETAILS),conn_array_index, type, this_circid, other_circid, circ->state,circuit_state_to_string(circ->state), (long)circ->timestamp_created.tv_sec);
    ...
    for (circ=global_circuitlist;circ;circ = circ->next) {
      if (! CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if (or_circ->p_conn == conn && or_circ->p_circ_id == circ_id) {
          log_warn(LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_CIRCUIT_NOT_IN_HASH_TABLE));
          return circ;
        }
      }
      if (circ->n_conn == conn && circ->n_circ_id == circ_id) {
        log_warn(LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_CIRCUIT_NOT_IN_HASH_TABLE_N));
        return circ;
      }
    }
    return NULL;
...
{	crypt_path_t *hop = circ->cpath;
	...
	do
	{	routerinfo_t *ri2;
		if(tor_memeq(hop->extend_info->identity_digest,info->identity_digest, DIGEST_LEN))	return 1;
		if(ri1 && (ri2 = router_get_by_digest(hop->extend_info->identity_digest)) && routers_in_same_family(ri1, ri2))	return 1;
		hop=hop->next;
...
{	circuit_t *_circ;
	...
	/* Make sure we're not trying to create a onehop circ by cannibalization. */
	tor_assert(!(flags & CIRCLAUNCH_ONEHOP_TUNNEL));
	log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITLIST_FIND_OPEN_CIRCUIT),purpose, need_uptime, need_capacity, internal);
	for(_circ=global_circuitlist; _circ; _circ = _circ->next)
	{	if(CIRCUIT_IS_ORIGIN(_circ) && _circ->state == CIRCUIT_STATE_OPEN && !_circ->marked_for_close && _circ->purpose == CIRCUIT_PURPOSE_C_GENERAL && !_circ->timestamp_dirty)
		{	origin_circuit_t *circ = TO_ORIGIN_CIRCUIT(_circ);
			if((!need_uptime || circ->build_state->need_uptime) && (!need_capacity || circ->build_state->need_capacity) && (internal == circ->build_state->is_internal) && circ->remaining_relay_early_cells && !circ->build_state->onehop_tunnel)
			{	if((!info || !info_in_cpath(info,circ)) && (!best || (best->build_state->need_uptime && !need_uptime)))	best = circ;
			}
		}
	}
	return best;
...
#ifdef int3
        if(!get_options()->MaxCircuitDirtiness) return;
#endif
...
{	circuit_t *circ;
	...
	{	if(CIRCUIT_IS_ORIGIN(circ) && !circ->marked_for_close)
			circ->timestamp_dirty = get_time(NULL) - options->MaxCircuitDirtiness-1;
	}
}
...
void circuit_expire_all_circs(DWORD exclKey)
{
  ...
  {	if (((conn->type == CONN_TYPE_OR) || (conn->type == CONN_TYPE_AP) || (conn->type == CONN_TYPE_EXIT))&& !conn->marked_for_close && (!exclKey || (conn->exclKey==exclKey)))
	{	if(CONN_IS_EDGE(conn))	connection_edge_end(TO_EDGE_CONN(conn), END_STREAM_REASON_DESTROY);
		if(conn->type == CONN_TYPE_AP)	connection_mark_unattached_ap(TO_EDGE_CONN(conn),END_STREAM_REASON_DESTROY);
                        ...
                        const char *file)
{
  ...
  if (circ->marked_for_close) {
    log(LOG_WARN,LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_DUPLICATE_CLOSE), file, line,circ->marked_for_close_file, circ->marked_for_close);
    return;
  }
  if (reason == END_CIRC_AT_ORIGIN) {
    if (!CIRCUIT_IS_ORIGIN(circ)) {
      log_warn(LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_INVALID_REASON),file, line, circ->purpose);
    }
  ...
  if (reason < _END_CIRC_REASON_MIN || reason > _END_CIRC_REASON_MAX) {
    if (!(orig_reason & END_CIRC_REASON_FLAG_REMOTE))
      log_warn(LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_REASON_OUT_OF_RANGE),reason,file,line);
    reason = END_CIRC_REASON_NONE;
  }
  ...
  if (circ->purpose == CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT) {
    ...
    log_info(LD_REND,get_lang_str(LANG_LOG_CIRCUITLIST_FAILED_INTRO_CIRC),safe_str(ocirc->rend_data->onion_address),safe_str(build_state_get_exit_nickname(ocirc->build_state)));
...
//  tor_assert(cp->port);
  ...
  switch (cp->state)
    ...
    case CPATH_STATE_AWAITING_KEYS:
      ...
      break;
    default:
      log_fn(LOG_ERR,LD_BUG,get_lang_str(LANG_LOG_CIRCUITLIST_UNEXPECTED_STATE),cp->state);
      tor_assert(0);
    }
...
{	time_t most_recent1=0,most_recent2=0,most_recent3=0;
	edge_connection_t *conn;
	circuit_t *circ;
	char *res1=NULL,*res2=NULL,*res3=NULL;
	res1=find_address(address);
	int i;
	if(res1)	return res1;
	smartlist_t *conns = get_connection_array();
	SMARTLIST_FOREACH_BEGIN(conns,connection_t *,c)
	{	if (c->type != CONN_TYPE_AP) continue;
		conn = TO_EDGE_CONN(c);
		circ = circuit_get_by_edge_conn(conn);
		if(circ && circ->purpose==CIRCUIT_PURPOSE_C_GENERAL && (conn->cpath_layer) && conn->socks_request->address)
		{	if(!stricmp(address,conn->socks_request->address))
			{	if(conn->_base.timestamp_created >= most_recent1)
				{	most_recent1 = conn->_base.timestamp_created;
					res1 = conn->cpath_layer ? conn->cpath_layer->extend_info->identity_digest : NULL;
				}
			}
			else if(!(AP_CONN_STATE_IS_UNATTACHED(conn->_base.state))&&!(conn->_base.state == AP_CONN_STATE_OPEN)&&(conn->_base.timestamp_created >= most_recent2))
			{	most_recent2 = conn->_base.timestamp_created;
				res2 = conn->cpath_layer ? conn->cpath_layer->extend_info->identity_digest : NULL;
			}
			else if(conn->_base.timestamp_created >= most_recent3)
			{	most_recent3 = conn->_base.timestamp_created;
				res3 = conn->cpath_layer ? conn->cpath_layer->extend_info->identity_digest : NULL;
			}
		}
	}
	SMARTLIST_FOREACH_END(conn);
	res1=res1?res1:(res2?res2:res3);
	if(res1)
	{	res2=tor_malloc(DIGEST_LEN*2+2);
		res3=res2;
		for(i=0;i<DIGEST_LEN;i++)
		{	*res2=(*res1&0xf0)>>4;
			*res2=*res2>9?*res2+7:*res2;*res2++ += 0x30;
			*res2=*res1++&0x0f;
			*res2=*res2>9?*res2+7:*res2;*res2++ += 0x30;
		}
		*res2=0;
...
{	circuit_t *circ;
	...
	for(circ=global_circuitlist;circ;circ = circ->next)
	{	tor_snprintf(nodeName,1023,"[%s][%s] %s_%d - %s",CIRCUIT_IS_ORIGIN(circ)?TO_ORIGIN_CIRCUIT(circ)->build_state->is_internal?"Internal":"Exit":"",(circ->magic==ORIGIN_CIRCUIT_MAGIC)?"Origin":(circ->magic==OR_CIRCUIT_MAGIC)?"OR":"Unknown",circuit_purpose_to_string(circ->purpose),(unsigned int)circ->n_circ_id,circuit_state_to_string(circ->state));
		tree_add_circ(circ,nodeName);
	}
	tor_free(nodeName);
}
...
{	circuit_t *circ;
	for(circ=global_circuitlist;circ;circ = circ->next)
	{	if(circ->hItem==hItem) return circ;
	}
	return NULL;
}
...
{	char *s=tor_malloc_zero(8192);
	...
	{	format_rfc1123_time(s2,(long)circ->timestamp_created.tv_sec);
		...
		{	char *s1 = circuit_list_path(TO_ORIGIN_CIRCUIT(circ),1);
			tor_snprintf(s,8192,get_lang_str(LANG_NETINFO_CIRCUIT_DUMP_2),n_circ_id,circuit_purpose_to_string(circ->purpose),circ->state,circuit_state_to_string(circ->state),s2,s1);
			tor_free(s1);
		}
	}
	else
