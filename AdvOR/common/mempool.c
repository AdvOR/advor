...
mp_pool_new(size_t item_size, size_t chunk_capacity)
{
  ...
  log_debug(LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS),(unsigned long)pool->new_chunk_capacity,(unsigned long)pool->item_alloc_size,(unsigned long)(pool->new_chunk_capacity*pool->item_alloc_size));
...
mp_pool_log_status(mp_pool_t *pool, int severity)
{
  ...
  log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_2),U64_PRINTF_ARG(bytes_allocated),pool->n_empty_chunks);
  for (chunk = pool->used_chunks; chunk; chunk = chunk->next) {
    ++n_used;
    bu += chunk->n_allocated * pool->item_alloc_size;
    ba += chunk->mem_size;
    log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_3),chunk->n_allocated);
  }
  log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_4),U64_PRINTF_ARG(bu),U64_PRINTF_ARG(ba), n_used);
  bytes_used += bu;
  bytes_allocated += ba;
  bu = ba = 0;
  ...
  log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_5),U64_PRINTF_ARG(bu),U64_PRINTF_ARG(ba),n_full);
  bytes_used += bu;
  bytes_allocated += ba;

  log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_6),U64_PRINTF_ARG(bytes_used),U64_PRINTF_ARG(bytes_allocated));

#ifdef MEMPOOL_STATS
  log_fn(severity,LD_MM,get_lang_str(LANG_LOG_MEMPOOL_STATS_7),U64_PRINTF_ARG(pool->total_items_allocated),U64_PRINTF_ARG(pool->total_chunks_allocated),U64_PRINTF_ARG(pool->total_chunks_freed));
#endif
}
#endif
