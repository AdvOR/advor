...
smartlist_uniq(smartlist_t *sl,
               int (*compare)(const void **a, const void **b),FREE_VAL)
{
  int i;
  for (i=1; i < sl->num_used; ++i) {
    if (compare((const void **)&(sl->list[i-1]),
      ...
      if (free_val)
#ifdef DEBUG_MALLOC
        free_val(sl->list[i],__FILE__,__LINE__);
#else
        free_val(sl->list[i]);
#endif
...
{
  smartlist_uniq(sl, _compare_string_ptrs, _tor_free_);
}
...
{
  smartlist_uniq(sl, _compare_digests, _tor_free_);
}
...
{
  smartlist_uniq(sl, _compare_digests256, _tor_free_);
}
 ...
 */
void
strmap_free(strmap_t *map, FREE_VAL)
{
...
#ifdef DEBUG_MALLOC
    if (free_val)
      free_val(this->val,__FILE__,__LINE__);
#else
    if (free_val)
      free_val(this->val);
#endif
 ...
 * <b>map</b>.
 */
void digestmap_free(digestmap_t *map,FREE_VAL)
{
